...
if uploaded_file is not None:
    ...
    for i in range(min(S_db.shape[1], frame_limit)):
        ...
        if shape_type == 0:
            ax1.plot(x, y_plot, color=plt.cm.plasma(sim), alpha=0.8, linewidth=2)
        elif shape_type == 1:
            ax1.fill(x, y_plot, color=plt.cm.viridis(sim), alpha=0.6)
        else:
            ax1.scatter(x, y_plot, color=plt.cm.inferno(sim), alpha=0.7, s=5)
