...
if uploaded_file is not None:
    ...
    for i in range(min(S_db.shape[1], frame_limit)):
        ...
        if shape_type == 'circle_fill':
            ...
            ax1.plot(xs, ys, color=plt.cm.magma(sim), alpha=0.7)
            ...
            for i in range(0, len(points), 4):
                for j in range(i, len(points), 5):
                    ax1.plot([points[i % len(points)][0], points[j % len(points)][0]],
                             ...
                             color=plt.cm.cividis(sim), alpha=0.2)
            ...
            ax1.plot(x_doodle, y_doodle, color=plt.cm.spring(sim), alpha=0.4, linewidth=1.5)
