# streamlit_dream_audio_advanced.py
...
if uploaded_file is not None:
    ...
    rms = librosa.feature.rms(y=y)[0]  # Root mean square (volume)
    ...
    for i in range(min(S_db.shape[1], 500)):
        pattern = S_db[:, i] / 80.0
        ...
        x = r * np.cos(t)
        y = r * np.sin(t)
        ax.plot(x, y, color=plt.cm.plasma(sim), alpha=0.8, linewidth=2)
        ax.set_xlim(-3, 3)
        ax.set_ylim(-3, 3)
        ax.set_title(f"Dream Frame {i+1} - Novelty: {1-sim:.2f}")
        ax.axis('off')
