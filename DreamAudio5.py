...
frame_count = st.slider("Select number of dream frames", min_value=50, max_value=1000, value=500, step=50)
...
if uploaded_file is not None:
    ...
    for i in range(min(S_db.shape[1], frame_count)):
        pattern = S_db[:, i] / 80.0
        vol = rms[i] if i < len(rms) else 0.5
        sim = net.train_on_sequence(pattern, reward=vol)
        signature.append(pattern.tolist())

        ax1.clear()
        shape_choice = random.choice(["circle", "star", "hexagon", "spiral"])
        t = np.linspace(0, 2 * np.pi, 64)
        r = 1 + 0.6 * np.array(pattern) * (1 + vol * 2)
        x = r * np.cos(t)
        y = r * np.sin(t)
        ...
        for unit in net.units:
            ...
            glow = unit.emotional_weight / 2
            ax2.plot(pos[0], pos[1], 'o', color=plt.cm.inferno(glow), alpha=0.6, markersize=4 + glow * 4)
        ax2.set_xlim(0, 1)
        ax2.set_ylim(0, 1)
    ...
    st.download_button("⬇️ Download Dream Audio", data=open(audio_path, "rb").read(), file_name="dream_audio.wav")